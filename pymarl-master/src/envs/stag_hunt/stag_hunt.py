...
class StagHunt(MultiAgentEnv):
    ...
    def __init__(self, batch_size=None, **kwargs):
        ...
        self.state_size = 5 * (self.n_agents + self.n_prey)
    ...
    def step(self, actions):
        ...
        # if self.args.need_render:
        #     matrix = np.zeros((self.args.world_shape))
        #     state = self.get_state()
        #     for i in range(self.n_agents):
        #         matrix[int(state[5 * i]), int(state[5 * i + 1])] = 1+0.1*i
        #     for i in range(self.n_stags):
        #         matrix[int(state[5 * self.n_agents + 5 * i]), int(state[5 * self.n_agents + 5 * i + 1])] = 3+0.1*i
        #     for i in range(self.n_hare):
        #         matrix[int(state[5 * (self.n_agents + self.n_stags) + 5 * i]), int(state[5 * (self.n_agents + self.n_stags) + 5 * i + 1])] = 5 +0.1*i
        #     fig = plt.figure()
    ...
    def get_state(self):
        ...
        if self.batch_mode:
            ...
            state = np.zeros(5 * (self.n_agents + self.n_prey))
            for a in range(self.n_agents):
                state[5 * a] = self.agents[a, 0, 0]
                state[5 * a + 1] = self.agents[a, 0, 1]
                state[5 * a + 2] = 1
            for s in range(self.n_stags):
                if self.prey_alive[s, 0] == 1:
                    state[5 * self.n_agents + 5 * s] = self.prey[s, 0, 0]
                    state[5 * self.n_agents + 5 * s + 1] = self.prey[s, 0, 1]
                    state[5 * self.n_agents + 5 * s + 3] = 1
            for h in range(self.n_hare):
                if self.prey_alive[self.n_stags + h, 0] == 1:
                    state[5 * (self.n_agents + self.n_stags) + 5 * h] = self.prey[self.n_stags + h, 0, 0]
                    state[5 * (self.n_agents + self.n_stags) + 5 * h + 1] = self.prey[self.n_stags + h, 0, 1]
                    state[5 * (self.n_agents + self.n_stags) + 5 * h + 4] = 1
            return state
